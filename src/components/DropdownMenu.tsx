...
const DropdownMenu = ({ children, childMenuData }: { children: React.ReactNode; childMenuData: any[] }) => {
    ...
    const renderSubMenu = (nodes: any[], parentNodes?: any[]) => {
        return (
            <div className="dropdown-menu p-2 bg-gray-50">
                {
    ...
    return <>
        ...
        <div className={`menuItem relative group cursor-pointer py-2 ${isClicked ? 'disable-hover' : ''}`}>


            {children}

            {childMenuData && childMenuData.length > 0 && (
                <div className="absolute bg-gray-50 megaMenu text-black" style={{ width: 'auto' }}>
                    <div className="p-4">
